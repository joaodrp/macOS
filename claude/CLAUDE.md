...
- Prefer individual or bulk operations through CLIs, MCP servers, or built-in tools over writing Bash/Python/AppleScript scripts. Scripts require explicit approval on each run and are harder to review.
...
- To request a review on a GitLab MR, post a comment with the quick action:
`/request_review @username`. Example: `glab mr note <MR_ID> --repo <repo> -m "/request_review @reviewer"`
